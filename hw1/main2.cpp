...
void *ThreadFunc(void *prm) // the function threads will execute
{
    ...
    stringstream s1, s2;
    s1 << (param->proCounter+1); // Changing int to string
    s2 << (param->thrCounter+1); // Changing int to string
    string thrName = "Thread" +s1.str() + "." + s2.str(); // set the threads name
...
int main(int argc, const char * argv[])
{
    ...
    if( pid !=0 )
    {
        waitpid (pid,NULL,0); // waits for slave1 to end
        type="Midterm";
        proCounter=1;
    ...
    if( pid !=0 )
    {
        waitpid (pid,NULL,0);// waits for slave2 to end
        type="Final";
        proCounter=2;
	...
	stringstream s;
   	s << (proCounter+1); // Changing int to string
	proName= "Slave" + s.str(); // sets the process's name
    ...
    waitpid (pid,NULL,0); // waits for slave3 to end
